...
class Multishipping
{
    ...
    protected function resetCheckout($quoteId)
    {
        ...
        if ($paymentIntentModel->getPiId())
        {
            try
            {
                ...
                if (!$this->paymentIntent->isSuccessfulStatus($paymentIntent))
                {
    ...
    public function placeOrder($quoteId)
    {
        ...
        try
        {
            ...
            $confirmParams = [
                'payment_method' => $multishippingQuoteModel->getPaymentMethodId()
            ];

            foreach ($successful as $order)
            {
                $this->paymentIntent->setTransactionDetails($order, $paymentIntent);
